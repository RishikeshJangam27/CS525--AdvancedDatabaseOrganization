...
extern RC writeBlock (int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage){
	...
	if(pageNum!=0){ // Write content to a non first page.
		...
		fseek(file_p,spec_pos,SEEK_SET);	
		int i;
		for(i=0;i<PAGE_SIZE;i++) 
		{
			if(feof(file_p)) // check file is ending in between writing
			{
				 appendEmptyBlock(fHandle); // append empty block at the end of file
			}
			fputc(memPage[i],file_p);// write content to file
		}
		fHandle->curPagePos = ftell(file_p);// set current file position to curPagePos 
		fclose(file_p); //closing filepointer	
	}	
...
extern RC ensureCapacity (int numberOfile_pages, SM_FileHandle *fHandle){
